...
<div x-data="{ deleteModal: false, isDeleting: false }" class="max-w-md mx-auto space-y-6">
    ...
    <div class="bg-white p-3 rounded-2xl border border-gray-200/80 shadow-2xs print:hidden space-y-2">
        <!-- Label row -->
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
            <button type="button" onclick="window.print()"
                class="shrink-0 px-3.5 py-2 rounded-full bg-[#324b3e] hover:bg-[#23382f] text-white text-xs font-bold shadow-md transition flex items-center gap-1.5 active:scale-[0.98]">
            ...
            <button type="button" id="btn-wa-share-due-slip"
                onclick="shareSlipImage('printable-due-slip', '<?= e($bill['bill_number']) ?>', '<?= rawurlencode($bill['customer_phone'] ?? '') ?>')"
                class="shrink-0 px-3 py-2 rounded-full bg-[#25d366]/10 hover:bg-[#25d366] text-[#128c4c] hover:text-white text-xs font-semibold border border-[#25d366]/40 transition flex items-center gap-1.5 active:scale-[0.98]"
            ...
            <button type="button" id="btn-copy-due-slip-img" onclick="copySlipImage('printable-due-slip', this)"
                class="shrink-0 px-3 py-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold transition flex items-center gap-1.5 active:scale-[0.98]"
                title="Copy slip image to clipboard to paste in WhatsApp">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                </svg>
                <span id="btn-copy-due-slip-img-text">Copy Image</span>
            </button>

            <a href="bills.php"
                class="shrink-0 px-3 py-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold transition">
                Bills
            </a>

            <a href="bill-slip.php?id=<?= $bill['id'] ?>"
                class="shrink-0 px-3 py-2 rounded-full bg-emerald-50 hover:bg-emerald-100 text-emerald-800 text-xs font-semibold border border-emerald-200 transition"
                title="View Full Bill">
                Full Bill
            </a>

            <button type="button" @click="deleteModal = true"
                class="shrink-0 btn-delete-animated px-3 py-2 rounded-full bg-rose-50 hover:bg-rose-600 hover:text-white text-rose-700 text-xs font-semibold border border-rose-200 transition flex items-center gap-1"
                title="Delete Bill">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div
        class="p-3.5 rounded-2xl border text-xs flex items-center justify-between print:hidden <?= ($bill['payment_status'] === 'unpaid' || (float) $bill['paid_amount'] <= 0) ? 'bg-rose-50 border-rose-200 text-rose-900' : 'bg-amber-50 border-amber-200 text-amber-900' ?>">
        <div class="flex items-center gap-2">
            <svg class="w-4 h-4 shrink-0 <?= ($bill['payment_status'] === 'unpaid' || (float) $bill['paid_amount'] <= 0) ? 'text-rose-600' : 'text-amber-600' ?>"
                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <?php if ($bill['payment_status'] === 'unpaid' || (float) $bill['paid_amount'] <= 0): ?>
                <span><strong>Fully Unpaid / Udhar Khata:</strong> Slip generated with ₹0 payment. All amount remains
                    outstanding under customer Khata.</span>
            <?php else: ?>
                <span><strong>Partial Due Active:</strong> Small slip issued for payment received. The official Red MOMAI
                    PLYWOOD Full Bill unlocks automatically once fully paid.</span>
            <?php endif; ?>
        </div>
    </div>

    <!-- Small Compact Slip Document -->
    <div id="printable-due-slip"
        class="bg-white rounded-2xl p-6 border-2 border-dashed border-gray-300 shadow-md text-gray-900 font-mono text-xs space-y-4 max-w-[380px] mx-auto">
        ...
        <div class="space-y-1 text-[11px] pb-3 border-b border-dashed border-gray-200">
            <div class="flex justify-between items-start gap-2">
                <span class="text-gray-500 shrink-0">Bill Ref:</span>
                <span class="font-bold text-gray-900 text-right break-all"><?= e($bill['bill_number']) ?></span>
            </div>
            <div class="flex justify-between items-start gap-2">
                <span class="text-gray-500 shrink-0">Date & Time:</span>
            ...
            <div class="flex justify-between items-start gap-2">
                ...
                <span class="font-bold text-gray-900 text-right break-words leading-tight"
            ...
            <?php if (!empty($bill['customer_phone'])): ?>
                <div class="flex justify-between items-start gap-2">
                    ...
                    <span class="text-gray-800 text-right"><?= e($bill['customer_phone']) ?></span>
                </div>
            <?php endif; ?>
            <?php if (!empty($bill['customer_address'])): ?>
                <div class="flex justify-between items-start gap-2">
                    <span class="text-gray-500 shrink-0">Address:</span>
                    <span class="text-gray-800 text-right break-words leading-tight"
                        style="overflow-wrap: anywhere; word-break: break-word;"><?= e($bill['customer_address']) ?></span>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="space-y-1.5 pb-3 border-b border-dashed border-gray-200 text-[11px]">
            ...
            <?php foreach ($items as $item): ?>
                <div class="flex justify-between items-start gap-2">
                    <div class="flex-1 min-w-0 pr-2" style="min-width: 0;">
                        <div class="font-semibold text-gray-800 break-words leading-snug"
                            style="overflow-wrap: anywhere; word-break: break-word;"><?= e($item['product_name']) ?></div>
                        <div class="text-gray-500 text-[10px] mt-0.5"><?= $item['quantity'] ?> x
                            <?= number_format((float) $item['unit_price'], 0) ?></div>
                    </div>
                    <span class="font-bold text-gray-900 text-right font-mono whitespace-nowrap shrink-0"
                        style="flex-shrink: 0;">₹<?= number_format((float) $item['total_price'], 0) ?></span>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <div class="space-y-1.5 pt-1 text-xs">
            <div class="flex justify-between text-gray-600">
                ...
                <span
                    class="font-bold text-gray-900 font-mono">₹<?= number_format((float) $bill['grand_total'], 2) ?></span>
            </div>
            ...
            <?php else: ?>
                <div
                    class="flex justify-between text-rose-700 font-bold bg-rose-50 px-2 py-1 rounded border border-rose-100">
                    <span>Payment Received Now:</span>
            ...
            <div class="flex justify-between items-baseline pt-2 border-t-2 border-gray-900 text-sm font-black">
                ...
                <span
                    class="text-rose-700 font-mono text-base">₹<?= number_format((float) $bill['due_amount'], 2) ?></span>
            </div>
        </div>
...
<script>
    ...
    /**
     * Copy slip image directly to clipboard for quick paste (Ctrl+V) into WhatsApp
     */
    async function copySlipImage(elementId, btnElement) {
        const btn = btnElement || document.getElementById('btn-copy-due-slip-img');
        const textSpan = document.getElementById('btn-copy-due-slip-img-text');
        ...
        try {
            ...
            alert('Could not copy image directly. Please use "Send via WhatsApp" or "Print Slip".');
            if (textSpan) textSpan.textContent = originalText;
        ...
        try {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                ...
                showToastNotice(copiedToClipboard
                    ? '📋 Slip image copied to clipboard! Press Ctrl+V in WhatsApp to paste and send.'
                    : '⬇️ Slip image downloaded! Please attach it in WhatsApp.');
            ...
            if (err.name !== 'AbortError') {
                ...
                alert('Could not share the slip. Please try printing or copying the image instead.');
            }
        } finally {
